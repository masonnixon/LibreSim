...
| FAC-8 | in progress | `5c3899c`, `2aa06ba`, `b34d2a0` | Strict CSV/key-set validation and a shared, stable output schema now reject empty, malformed, duplicate, missing, unexpected, nonfinite, and shape-mismatched output instead of accepting positional or empty comparisons. All 156 archives regenerated; focused validation tests passed 27/27, the existing codegen suite passed 392/392, Ruff/mypy passed, and the 2026-07-14 full four-language baseline completed with no build failures and 52/156 semantic passes. Declared source-port dimensions now prevent scalar ports on vector-capable blocks from expanding into whole-block vectors: all nine prior simulation-shape failures reached comparison, with 24/36 targeted implementations newly passing and the other 12 classified as numerical mismatches. The focused simulator/compiler/validator suite passed 143/143. Remaining families are 16 approved-empty-analysis decisions and shared numerical mismatches. |
