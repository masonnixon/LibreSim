...
`16c97ab`, and authorized production work after the pre-FAC-9 checkpoint. Phases 0-6
are complete through `4df7b63`; compatibility and closeout are next.
...
| FAC-9 | in progress; Phases 0-6 complete | `e536fb4`, `16c97ab`, `f3051b9`, `9d4531a`, `a450f20`, `ec15bb1`, `e33a9fd`, `3898ed5`, `959e8aa`, `cc0fc70`, `4df7b63` | 2026-07-16 maintainer selected and approved concurrent simulations. Phase 0 reproduced deterministic solver/clock cross-talk. Phase 1 added the instance-owned `SimContext`, compatibility facade, adapter ownership, and isolation regressions. Phase 2 bound blocks and registered integrators to one graph/context owner, made native `Sim` instance-scoped, paired legacy fallbacks, and proved different-solver native simulations overlap safely in threads. Phase 3 normalized nonzero start/reset lifecycles, activated reporting/state boundaries, proved deterministic adapter and event-gated runner interleavings match isolated references, and added exact opaque per-runner operation ownership with atomic scheduling, deterministic API conflicts, committed-boundary pause acknowledgment, exact enter-step handoff, ABA protection, and cancellation cleanup. Phase 4 migrated every built-in mutable timing/readiness access to its owned context and added deterministic ABAB/AABB coverage for Sine, Clock, RateLimiter, LowPassFilter, UnitDelay, and TransportDelay. Phase 5 added frozen versioned adapter/runner checkpoints with canonical fingerprints, complete context/integrator state, explicit coverage for all 181 built-in codecs, compact internal histories, detached public captures, pre-mutation validation, atomic rollback, operation ownership, deterministic state-zoo replay, and peer isolation. Phase 6 added a bounded process-local session registry, owned task/token finalization, leased targeting, tombstoned deletion/promotion, safe pruning, default replacement serialization, opt-in coexistence, optional targeting across every read/control/step route, and matching frontend client options. Deterministic API tests cover live peers, unknown IDs, same-session conflicts, replacement/coexistence, deletion, capacity races, failed initialization, and pre-start cancellation. The Phase 6 Docker gates passed 2,185 backend tests with 1 skip and 664 frontend tests; Ruff/mypy/ESLint/TypeScript passed. Phase 7 compatibility and closeout are next. |
