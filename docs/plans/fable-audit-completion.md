...
| FAC-9 | in progress; Phases 0-2 complete | `e536fb4`, `16c97ab`, `f3051b9`, `9d4531a` | 2026-07-16 maintainer selected and approved concurrent simulations. Phase 0 reproduced deterministic solver/clock cross-talk for alternating adapters and runners. Phase 1 added the instance-owned `SimContext`, compatibility facade, adapter activation/ownership, and isolation regressions. Phase 2 atomically binds blocks and registered integrators to one graph/context owner, makes native `Sim` execution instance-scoped, pairs the sequential legacy `Sim.*` and `State.*` fallbacks, and proves two different-solver native simulations overlap safely in threads. The Phase 2 Docker gate passed 2,130 tests with 1 skip; Ruff and mypy passed across 136 source files; the thread regression passed 20 consecutive repetitions. Phase 3 runner operation ownership is next. |
