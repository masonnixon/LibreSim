...
| FAC-8 | in progress | `5c3899c`, `2aa06ba`, `b34d2a0`, `d7dc2d1`, `7ed9e45` | Strict CSV/key-set validation and a shared, stable output schema now reject empty, malformed, duplicate, missing, unexpected, nonfinite, and shape-mismatched output instead of accepting positional or empty comparisons. Declared source-port dimensions eliminated all nine simulation-shape failures. Generated runtimes now separate the OSK major/ready update from integration stages, hold ready-only noise/discrete/observer state during RK passes, use the runtime step plus canonical parameter names for rate limiting, and propagate PI, PD, and model-reference continuous state in every target language. Generated ZIP metadata is fixed so repeated canonical regeneration is byte-reproducible. The focused codegen suite passed 417/417 with Ruff and mypy clean; all 156 archives regenerated, built, and executed. The 2026-07-14 full matrix improved from 52/156 to 89/156 semantic passes (57.1%) with zero build or simulation failures. Remaining failures are 16 empty analysis targets awaiting an approved contract and 51 shared or language-specific numerical mismatches. |
